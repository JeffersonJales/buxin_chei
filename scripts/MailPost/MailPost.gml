...
/// For more info https://github.com/JeffersonJales/Mail_Post_GM
...
function MailPost(instance_or_struct_is_persistant = false, inst_struct_reference = other, prevent_same_events = false) constructor {
	...
	prevent_same_events_on_events_list = prevent_same_events;
	
	mailpost_subscriptions = []; 
	
	/// @func add_subscription
	static add_subscription	= function(event, callback_function, data = undefined){ 
		
		if(prevent_same_events_on_events_list){
			var _i = 0; 
			repeat(array_length(mailpost_subscriptions)){
				if(mailpost_subscriptions[_i].__event == event) return self;
				_i++;
			}
		}
		
		var _sub_data = new __mailpost_subscription_data(event, callback_function, scope_reference, data, self);
		array_push(mailpost_subscriptions, _sub_data);
...
function mailpost_delivery(event, broadcast_data = undefined){
	...
	for(var _i = 0; _i < ds_list_size(_subscribers); _i++){
		...
		if(_subscription_data == undefined) 
			continue;
...
function mailpost_delete(mailpost_object){
...
function __mailpost_force_delete_queue(){
	...
	while(_mailpost_data != undefined){
		...
		var _event = _mailpost_data.__event
		var _list = global.__mp_subscriptions_by_event[? _event];
		
		if(_list != undefined){
			ds_list_delete_search(_list, _mailpost_data);
		}
		
		var _mailpost = _mailpost_data.__mailpost_reference;
		if(is_array(_mailpost.mailpost_subscriptions)){
			_mailpost.mailpost_subscriptions = array_remove(_mailpost.mailpost_subscriptions, _mailpost_data);
		}
